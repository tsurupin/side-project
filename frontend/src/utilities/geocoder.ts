...
export const fetchAddress = async (
  ...
  try {
    ...
    const streetAddress: StreetAddress = results.filter(({ types }) => types.includes(STREET_ADDRESS_TYPE))[0];
    ...
    let cityName;
    let countryName;
    streetAddress.address_components.forEach(({ types, long_name, short_name }) => {
